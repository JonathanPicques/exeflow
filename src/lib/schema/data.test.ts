...
test('zero', () => {
    ...
    expect(zero({type: 'string', editor: {suggestions: ['what is up?']}})).toBe('what is up?');
    expect(zero({type: 'string', const: 'hello!', default: 'welcome!', editor: {suggestions: ['what is up?']}})).toBe('hello!');
